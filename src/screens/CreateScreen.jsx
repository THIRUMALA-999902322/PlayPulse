...
const CreateScreen = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (dbError) {
      // Still show success — app works even without DB for demo
  ...
  if (submitted) {
    return (
      <div className="screen" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 40, textAlign: "center" }}>
        ...
        <div style={{ color: theme.textMuted, fontSize: 14, marginBottom: 24 }}>Your match is now live on the map. Nearby players will be notified.</div>
    ...
    <div className="screen scroll-area">
      ...
      <div className="create-screen">
        ...
        <div className="form-group">
          ...
          <input className="form-input" placeholder="📍 Search or enter location" value={location} onChange={e => setLocation(e.target.value)} />
        </div>
        ...
        <div className="form-group">
          ...
          <div className="toggle-group">
            <button className="toggle-btn">📱 In-App Stream</button>
            <button className="toggle-btn">▶ YouTube</button>
          </div>
        </div>

        <button className="submit-btn" onClick={handleSubmit} disabled={loading}>
          {loading ? "Posting..." : "⚡ Post Match Now"}
        </button>
      </div>
    </div>
  );
